...
def task_template_resources(task_template_version: int, workflows: Optional[list] = None,
                            node_args: Optional[Dict] = None, requester_url: Optional[str] = None,
                            ci: Optional[float] = None) -> Tuple:
...
def get_task_dependencies(task_id: int, requester_url: Optional[str] = None) -> dict:
    ...
    # Valid input
    rc, res = requester.send_request(
        app_route=f"/task_dependencies/{task_id}",
        message={},
        request_type="get")
    if rc != 200:
        if rc == 404:
