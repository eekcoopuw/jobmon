...
def task_template_resources(task_template_version, workflows=None,
                            node_args=None, requester_url=None, ci=None):
...
def update_task_status(task_ids: List[int], workflow_id: int, new_status: str,
    ...
    if force and recursive:
        rc, res = requester.send_request(
            app_route="/cli/tasks_recursive/" + ("up" if new_status == "D" else "down"),
    ...
    if len(task_ids) > 10_000:
        raise AssertionError(f"There are too many tasks ({len(task_ids)}) requested "
...
def validate_workflow(task_ids: List[int], requester: Requester, force: bool = False)\
        -> WorkflowStatus:
    """
    ...
    if not bool(res["validation"]):
        raise AssertionError("The workflow status of the given task ids are out of "
...
def get_task_dependencies(task_id: int, requester_url: Optional[str] = None) -> dict:
    ...
    # Valid input
    rc, res = requester.send_request(
        app_route=f"/cli/task_dependencies/{task_id}",
        message={},
        request_type="get")
    if rc != 200:
        if rc == 404:
...
def workflow_reset(workflow_id: int, requester: Requester) -> str:
    ...
    rc, res = requester.send_request(
        app_route=f"/cli/workflow/{workflow_id}/validate_for_workflow_reset/{username}",
        message={},
        request_type="get",
        logger=logger)
    if rc != 200:
    ...
    if res["workflow_run_id"]:
        rc, _ = requester.send_request(
            app_route=f"/cli/workflow/{workflow_id}/reset",
            message={},
            request_type="put",
            logger=logger)
        if rc != 200:
        ...
        wr_return = f"User {username} is not the latest initiator of " \
...
def _get_yaml_data(wfid: int, tid: int, v_mem: str, v_core: str, v_runtime: str,
                   requester: Requester):
    ...
    rc, res = requester.send_request(
        ...
        logger=logger)
    if rc != 200:
        raise AssertionError(f"Server returns HTTP error code: {rc} "
                             f"for get_task_template_version.")
    ttvis_dic = dict()
    ...
    for ttv in ttvis_dic.keys():
        ...
        if rc != 200:
            raise AssertionError(f"Server returns HTTP error code: {rc} "
                                 f"for /client/task_template_resource_usage.")
    ...
    if rc != 200:
        raise AssertionError(f"Server returns HTTP error code: {rc} "
                             f"for /cli/get_most_popular_queue.")
...
def create_resource_yaml(wfid: int, tid: int, v_mem: str, v_core: str, v_runtime: str,
                         clusters: List, requester_url: Optional[str] = None) -> str:
