...
class DistributorWorkflowRun:
    ...
    def prep2(self):
        retry_later: DistributorTaskInstance = []
        while self._capacity > 0 and self.instantiated_task_instances:
            task_instance = self.instantiated_task_instances.pop(0)
            array = self.get_array(task_instance.array_id)

            try:
                # TODO: move capacity logic into array
                array.queue_task_instance_id_for_array_launch(task_instance.task_instance_id)
            # TODO: change to custom exception
            except Exception:
                retry_later.append(task_instance)

        self.instantiated_task_instances.append(retry_later)
