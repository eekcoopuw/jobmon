...
def get_task_template():
    ...
    if tt is not None:
        ...
        app.logger.debug(f"Got task template {tt.id} for tool_version_id {tool_version_id}")
    else:
...
def add_task_template():
    ...
    try:
        ...
        DB.session.add(tt)
        DB.session.commit()
        app.logger.debug(f"Task template {tt.id} added for tool_version_id {tool_version_id}")
...
def get_task_template_version(task_template_id: int):
    ...
    if ttv is not None:
        ...
        app.logger.debug(f"Got a task template version for TT {task_template_id}: {wire_obj}")
    else:
        wire_obj = None
