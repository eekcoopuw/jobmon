...
def get_queues_by_cluster_name(cluster_name: str):
    ...
    query = """
        SELECT q.id AS queue_id, q.name AS queue_name, c.name AS cluster_name, ct.name AS cluster_type_name, q.parameters
        FROM queue q
...
def get_queue_by_cluster_queue_names(cluster_name: str, queue_name: str):
    """Get the id, name, cluster_name and parameters of a Queue based on cluster_name and queue_name."""
