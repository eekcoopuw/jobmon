...
class SchedulerLogging():
    ...
    def attach_log_handler(tag: str):
        ...
        logger.setLevel(SchedulerLogging._logLevel)
        formatter = logging.Formatter(tag + SchedulerLogging._format)
        hs = logging.StreamHandler()
        hs.setFormatter(formatter)
        logger.addHandler(hs)
        if SchedulerLogging._syslogAttached:
            p = socket.SOCK_DGRAM
...
SchedulerLogging.attach_log_handler("JOBMON_SCHEDULER")
