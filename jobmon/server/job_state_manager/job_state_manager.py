...
@jsm.route('/job_instance/<workflow_run_id>/nonterminal_to_k_status',
           methods=['POST'])
def ji_nonterminal_state_to_k_state(workflow_run_id: int):
    ...
    query = """
            ...
            AND (status = (:batch_executor) OR status = (:instantiated) OR 
            status = (:running))"""
...
@jsm.route('/job_instance/<executor_id>/maxpss/<maxpss>', methods=['POST'])
def set_maxpss(executor_id: int, maxpss: int):
    ...
    try:
        sql = f"UPDATE job_instance SET maxpss={maxpss} WHERE executor_id={executor_id}"
