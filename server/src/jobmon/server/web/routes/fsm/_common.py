...
class IgnoreFormatter(dict):
    def __missing__(self, key: str) -> str:
        """Returns specified value when there is a key missing in dict."""
        return "{" + key + "}"


def _get_logfile_template(
    task_resources_id: int, template_type: str, session: Session
) -> Tuple[Dict, str]:
    ...
    log_templates, requested_resources, queue_name = session.execute(
        select_stmt
    ).fetchone()
        ...
        for template_type in template_types:
            if template_type in log_templates and log_type in requested_resources:
                if log_type in log_templates[template_type]:
                    ...
                    ignore_missing_mapper = IgnoreFormatter(
                        root=requested_resources[log_type]
                    )
                    log_format = log_templates[template_type][log_type]
                    log_type_dict[template_type] = log_format.format_map(
                        ignore_missing_mapper
                    )
