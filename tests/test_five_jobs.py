...
def BROKEN_test_five_jobs():
    ...
    path_to_conda_bin_on_target_vm = os.getenv(
        'CONDA_ROOT',
    ...
    try:
        ...
        for i in [i for i in range(5)]:
            logger.info('{}: test loop qsub {}'.format(
                os.getpid(), "{root}/tests/mock_job.py".format(root=root)))
            q.qsub(
        ...
        logger.debug('    {}: test_five_jobs qblock complete'.format(
                os.getpid()))
        ...
        logger.info('{}: Number of failed jobs {}'.format(
                os.getpid(), number_failed))
        ...
        if number_failed != 2:
            raise Exception("Too many jobs failed "
                            "({}), expected 2 failures".format(number_failed))
    except Exception as e:
        logger.error("test_five_jobs failed with exception '{}'".format(e))
        assert False
    finally:
        logger.info('{}: test_five_jobs stopping the monitor'.format(
                os.getpid()))
