...
def test_instantiate_queued_tasks(
    ...
    array2 = array_template.create_array(
        arg=[4, 5, 6], cluster_name="sequential", compute_resources={"queue": "null.q"}
    )
    ...
    with app.app_context():
        ...
        """.format(
            distributor_wfr.workflow_run_id
        )
...
def get_batch_number(db_cfg, task_instance_id):
    ...
    with app.app_context():
        query = (
            f"SELECT array_batch_num FROM task_instance WHERE id = {task_instance_id}"
        )
        resp = DB.session.execute(query).fetchone()[0]
    return resp
...
def test_distributor_launch(tool, db_cfg, client_env, task_template, array_template):
    ...
    _, _ = requester._send_request(
        app_route=f"/task/{task_1.task_id}/queue", message={}, request_type="post"
    )
    ...
    single_task_id = distributor_wfr.launch_task_instance(
        task_instance=dtis_4, cluster=distributor
    )
...
def test_array_distributor_launch(
    tool, db_cfg, client_env, task_template, array_template
):
    from jobmon.serializers import SerializeClusterType
    ...
    array_id = distributor_wfr.launch_array_instance(
        array=distributor_array, cluster=distributor
    )
    single_task_id = distributor_wfr.launch_task_instance(
        task_instance=dtis_4, cluster=distributor
    )
    ...
    assert (
        call_get_array_task_instance_id(
