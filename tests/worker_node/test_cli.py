...
def test_unwrap_happy_path(client_env):
    ...
    with patch.dict(os.environ, {"JOB_ID": "77777"}), patch(
    ...
    ) as m_done, patch(worker_node + WNTI + "log_running") as m_run, patch(
        worker_node + WNTI + "log_report_by"
    ) as m_by, patch(
        worker_node + WNTI + "log_error"
    ) as m_err, patch(
        worker_node + WNTI + "in_kill_self_state"
    ) as m_kill_self, patch(
        worker_node + "worker_node_task_instance.kill_self"
    ) as m_kill:
        ...
        worker_node_ti = worker_node_task_instance.WorkerNodeTaskInstance(
            task_instance_id=1,
...
def test_stderr_buffering(capsys, client_env):
    ...
    with patch.dict(os.environ, {"JOB_ID": "77777"}), patch(
        ...
        m_run.return_value = (
            200,
            "message",
            f"python {os.path.join(thisdir, 'fill_pipe.py')}",
        )
