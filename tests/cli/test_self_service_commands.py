...
def test_task_reset(db_cfg, client_env, monkeypatch):
    ...
    validate_username(workflow.workflow_id, getpass.getuser(), requester)
...
def test_update_task_status(db_cfg, client_env):
    ...
    from jobmon.client.status_commands import update_task_status, workflow_tasks
    ...
    # now that all statuses are D, we set the first one as G, and run -f and -r
    update_str = f'update_task_status -w {wf1.workflow_id} -t {wf1_tasks[0].task_id} -s G -f -r'
    args = cli.parse_args(update_str)
    update_task_status(task_ids=args.task_ids, workflow_id=args.workflow_id,
                       force=args.force, recursive=args.recursive,
                       new_status=args.new_status)

    # we should get 5 tasks with G status
    # (couldn't use workflow_tasks -s REGISTERED as REGISTERED is excluded in worflow_status
    # as invalid param).
    app = db_cfg["app"]
    DB = db_cfg["DB"]
    with app.app_context():
        ...
        WHERE workflow_id = :workflow_id
            AND status = 'G'"""
        res = DB.session.execute(sql, {"workflow_id": wf1.workflow_id}).fetchone()
        DB.session.commit()
    assert res[0] == 5

    # now that all statuses are G, we set the last one as D, and run -f and -r
    update_str = f'update_task_status -w {wf1.workflow_id} -t {wf1_tasks[4].task_id} -s D -f -r'
    args = cli.parse_args(update_str)
    update_task_status(task_ids=args.task_ids, workflow_id=args.workflow_id,
                       force=args.force, recursive=args.recursive,
                       new_status=args.new_status)

    # we should get 5 tasks with D status
    command_str = f"workflow_tasks -w {wf1.workflow_id} -s DONE"
    args = cli.parse_args(command_str)
...
def test_chunk_size(test_input, expected):
    from jobmon.client.status_commands import _chunk_ids
    r = _chunk_ids(test_input)
    assert r == expected