...
# @pytest.mark.skip(
#     reason="This is a unit test, not an integration test."
# )
def test_get_slurm_resource_usages_via_slurm_sdb():
    ...
    from jobmon.server.usage_integration.usage_utils import QueuedTI
    from jobmon.server.usage_integration.usage_integrator import UsageIntegrator
    from jobmon.server.usage_integration.usage_integrator import _get_squid_resource_via_slurm_sdb

    usage_integrator = TUsageIntegrator()

    distributor_ids = [201, 223, 25420, 595825, 595826, 595827]
    qtis = [QueuedTI(
        task_instance_id=distributor_id,  # fake task_instance_id
        distributor_id=distributor_id,
        cluster_type_name="slurm",
        cluster_id=1,
    ) for distributor_id in distributor_ids]

    all_usage_stats = _get_squid_resource_via_slurm_sdb(
        session=usage_integrator.session_slurm_sdb,
        tres_types=usage_integrator.tres_types,
        task_instances=qtis
    )

    print(f"all_usage_stats = {all_usage_stats}")
